...
#include "openvslam/imu/internal/inertial_gravity_scale_edge_on_camera_wrapper.h"
...
global_inertial_gravity_scale_ba::global_inertial_gravity_scale_ba(data::map_database* map_db, const unsigned int num_iter, const bool use_huber_kernel)
    : map_db_(map_db), num_iter_(num_iter), use_huber_kernel_(use_huber_kernel) {}

void global_inertial_gravity_scale_ba::optimize(Sophus::SO3d & Rwg_, double& s, const unsigned int lead_keyfrm_id_in_global_BA, bool* const force_stop_flag,
                  double info_prior_acc, double info_prior_gyr) const {
...
//    auto algorithm = new g2o::OptimizationAlgorithmGaussNewton(std::move(block_solver));
    algorithm->setUserLambdaInit(10);
    ...
    for (const auto keyfrm : keyfrms) {
        ...
        velocity_vtx->keyframe_id_ = keyfrm->id_;
//        std::cout<<"velocity_vtx->keyframe_id_: "<<velocity_vtx->keyframe_id_<<std::endl;
...
//        lm_vtx->setFixed(true);
    ...
    auto Rwg_vtx = new imu::internal::gravity_dir_vertex();
    Rwg_vtx->setId(*(vtx_id_offset)+1);
    Rwg_vtx->setEstimate(Rwg_);
    optimizer.addVertex(Rwg_vtx);
//    Rwg_vtx->setFixed(true);

    auto scale_vtx = new imu::internal::scale_vertex();
    scale_vtx->setId(*(vtx_id_offset)+2);
    scale_vtx->setEstimate(s);
    optimizer.addVertex(scale_vtx);
//    scale_vtx->setFixed(true);
    std::cout<<"scale before: "<<scale_vtx->estimate()<<std::endl;
    std::cout<<"gravity before: "<<Rwg_vtx->estimate().matrix()<<std::endl;
    ...
    for (size_t i = 0; i < keyfrms.size(); i++) {
        ...
        imu::internal::bias_vertex* acc_bias_vtx1 = acc_bias_vtx_container.get_vertex(ref_keyfrm);
        imu::internal::bias_vertex* gyr_bias_vtx1 = gyr_bias_vtx_container.get_vertex(ref_keyfrm);
        imu::internal::bias_vertex* acc_bias_vtx2 = acc_bias_vtx_container.get_vertex(keyfrm);
        imu::internal::bias_vertex* gyr_bias_vtx2 = gyr_bias_vtx_container.get_vertex(keyfrm);
        ...
        imu::internal::inertial_gravity_scale_edge_on_camera_wrapper wrap(keyfrm->imu_preintegrator_from_inertial_ref_keyfrm_->preintegrated_,
                                                                          acc_bias_vtx1, gyr_bias_vtx1,
                                                                          keyfrm_vtx1, velocity_vtx1, keyfrm_vtx2, velocity_vtx2,
                                                                          Rwg_vtx,scale_vtx);

        optimizer.addEdge(wrap.edge_);
...
//    optimizer.optimize(1);
    ...
    std::cout<<"scale after: "<<scale_vtx->estimate()<<std::endl;
    std::cout<<"gravity after: "<<Rwg_vtx->estimate().matrix()<<std::endl;
    ...
    s = scale_vtx->estimate();
    Rwg_ = Rwg_vtx->estimate();
    ...
    // 6. Extract the result
    {
        ...
        for (auto keyfrm : keyfrms) {
            ...
            if (keyfrm->inertial_ref_keyfrm_ || keyfrm->inertial_referrer_keyfrm_) {
